...
const maxFotos = 30; // cambia según cuántas tengas

// CARGAR IMÁGENES AUTOMÁTICAMENTE
for (let i = 1; i <= maxFotos; i++) {
    const img = new Image();
    img.src = carpeta + prefijo + i + extension;
    ...
    img.onload = function () {
        ...
        card.innerHTML = `
            <img src="${img.src}" onclick="abrir(this.src)">
            <p>Actividad ${i}</p>
        `;
...
function moverCarrusel() {
    ...
    if (indice < 0) indice = tarjetas.length - 1;
    if (indice >= tarjetas.length) indice = 0;
...
carrusel.addEventListener("touchstart", e => {
    ...
    moviendo = true;
    clearInterval(auto); // pausa mientras arrastras
});
...
carrusel.addEventListener("touchend", e => {
    ...
    if (diferencia > 50) {
        indice++; // deslizó a la izquierda
    } else if (diferencia < -50) {
        indice--; // deslizó a la derecha
    }

    moverCarrusel();
    moviendo = false;

    // reanudar auto-carrusel
    auto = setInterval(() => {
        indice++;
        moverCarrusel();
    }, 3500);
